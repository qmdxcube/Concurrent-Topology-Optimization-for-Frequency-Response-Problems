...
% damping=0.008;% damping ratio
Db=E0/(1-nu^2)*[1 nu 0; nu 1 0; 0 0 (1-nu)/2];
%% Definition of design domain and volume fraction
lx=6.0;ly=3.0;
nelx=600;nely=300;
nelx_mic=500;nely_mic=500;
vfmax=0.2;vfmax_mic=0.4; % volume fraction of total and micro design domain
vfmax_mac=vfmax/vfmax_mic; % volume fraction of macro design domain
...
G(2*(nely+1)*(nelx+1),1)=3000;
L=sparse(2*(nely+1)*(nelx+1),1); % The target dof in the problem
L(2*(nely+1)*(nelx+1),1)=1;
sOmega=0;eOmega=100*pi; % The frequency interval
num_Omega=400;
%% Filter radius
rmin=30;rmin_mic=50;
